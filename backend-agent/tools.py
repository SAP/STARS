...
@tool
def start_easyjailbreak_attack(
    attack_name: str,
    mutate_model_name: str,
    target_model_name: str,
    eval_model_name: str
) -> str:
    """
    Use this function to start an attack using the easyjailbreak framework.
    Run easyjailbreak_how before running this function. Some attacks need
    different parameters.
    @params
    attack_name: Since easyjailbreak supports many attacks, the name of the
    attack must be specified here.
    mutate_model_name: The name of the model used in the attack to mutate
    prompts.
    ...
    return str(AttackSpecification.create(
        ...
        target=target_model_name,
        attack_model=mutate_model_name,
        eval_model=eval_model_name).start())
